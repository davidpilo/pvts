...
public class TextIO {
   ...
   // public static char getAnyChar(){ return readChar(); }
    public static char getAnyChar(){ char c = readChar();
    if (c == '\n' && PVTS.bufferStack.size() == 2) {PVTS.currentLine ++;} return c; }
    ...
    public static String getWord() {
     ...
     public static String getWordForSEString() {
        char ch = lookChar();
        StringBuffer str = new StringBuffer(50);
        while (ch!='"') {
            //if (TextIO.peek() != '~') {
    ...
    public static String buffer = null;  // one line read from input
    public static int pos = 0;           // position of next char in input line that has
    ...
    public static void fillBuffer(String st, int p) {
        buffer = st+"\n"+"\n"+" "+'~';
        TextIO.setPos(p);
        //TextIO.putln(buffer);
        // in = st.toInputStream();
